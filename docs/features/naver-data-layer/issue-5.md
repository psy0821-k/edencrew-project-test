...
**전체 Phase 1 소요(커밋 시각 기준, 원시값)**: `4fa3253`(13:13) ~ `3a96660`(21:02) 약 7시간 49분.

**실질 작업시간(추정)**: 위 구간 중 점심·저녁 식사로 비운 약 2시간을 제외하면 **약 5시간 49분**으로, 시간 상한 합계(1+1+1.5+3=6.5시간) **이내**. 이 2시간은 커밋 로그 등으로 실측한 값이 아니라 개발자 본인의 기억에 근거한 추정치이며, 정확한 시각 기록은 남아있지 않다. 커밋 시각 간격 자체에도 요구사항 설계 논의, 사용자 질문 응답, 워크플로우 정리(이슈/브랜치/PR 전환) 시간이 섞여 있어 "구현에만 쓴 시간"과는 다소 차이가 있을 수 있다.
